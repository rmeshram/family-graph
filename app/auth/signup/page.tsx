      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="At least 6 characters"
            ...
            className="pl-10 pr-10 h-11 bg-muted/50 border-border" required minLength={6} />
...
// ─── Phone OTP signup (same flow as signin — Supabase auto-creates account) ───
...
function friendlyPhoneError(message: string): string {
  ...
  if (m.includes('unsupported phone provider') || m.includes('phone provider')) {
    return 'SMS delivery is not configured yet. Please sign up with email or Google for now.'
  }
...
function PhoneSignUp({ onSwitchToEmail }: { onSwitchToEmail?: () => void }) {
  const router = useRouter()
  const searchParams = useSearchParams()
  const supabase = createClient()
        ...
        <>
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <div className="flex items-center justify-center rounded-lg border border-border bg-muted/50 px-3 h-11 text-sm font-medium shrink-0 gap-1.5">
                🇮🇳 <span className="text-muted-foreground">+91</span>
              </div>
              <div className="relative flex-1">
                <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                <Input id="phone-signup" type="tel" inputMode="numeric" placeholder="98765 43210" value={phone}
                  onChange={e => setPhone(e.target.value.replace(/\D/g, '').slice(0, 10))}
                  className="pl-10 h-11 bg-muted/50 border-border font-mono tracking-wide" autoFocus />
              </div>
            </div>
            <p className="text-xs text-muted-foreground">No password needed — we'll verify via OTP.</p>
          </div>
      ...
      {step === 'otp' && (
        <>
          ...
          <Button className="w-full h-11" disabled={isLoading || otp.length !== 6} onClick={verifyOtp}>
            {isLoading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Verifying…</> : <>Verify & Continue <ArrowRight className="w-4 h-4 ml-2" /></>}
          </Button>
          <div className="text-center">
            {resendSeconds > 0
...
function SignUpContent() {
  ...
  const signUpWithGoogle = async () => {
    ...
    const callbackUrl = nextParam && nextParam.startsWith('/') && !nextParam.startsWith('//')
  ...
  return (
    <div>
      ...
      {googleError && (
        <div className="mb-3 p-3 rounded-lg bg-destructive/10 border border-destructive/20 text-destructive text-sm">{googleError}</div>
      )}
      <Button
        type="button"
        variant="outline"
        className="w-full h-12 border-border hover:bg-muted font-semibold text-base mb-3"
      ...
      <div className="relative my-5">
        <Separator />
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-background px-2 text-xs text-muted-foreground">or sign up with</span>
      </div>
      ...
      {showPhone && tab === 'phone' && <PhoneSignUp onSwitchToEmail={showTabs ? () => setTab('email') : undefined} />}
      {showEmail && tab === 'email' && <EmailSignUp />}
