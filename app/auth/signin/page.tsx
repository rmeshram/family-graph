...
function EmailSignIn() {
  ...
  return (
    <>
      <Button type="button" variant="outline" className="w-full h-11 border-border hover:bg-muted mb-4" onClick={signInWithGoogle} disabled={isLoading}>
      ...
      <div className="relative mb-6">
        <Separator />
        <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-background px-2 text-xs text-muted-foreground">or sign in with email</span>
      </div>
  ...
  return (
    <div>
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground text-sm">Sign in to find your match</p>
      </div>
      ...
      <Link href="/matches?demo=1"
        className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg border border-pink-500/30 bg-pink-500/[0.07] py-2.5 text-sm font-medium text-pink-400 hover:bg-pink-500/15 transition-colors">
        Browse matrimony profiles — no sign in needed
      </Link>
    </div>
  )
}
