...
function generateAIResponse(query: string, members: FamilyMember[]): { content: string; relatedIds: string[] } {
  ...
  if (q.includes('maternal side') || q.includes('nana') || q.includes('nani') || q.includes('mother side')) {
    ...
    return {
      content: `🌿 **Maternal Side of Your Family (${maternalMembers.length} members):**\n\n${maternalMembers.map(m => `• **${m.name}** (${m.relationship}) — ${m.currentPlace || m.birthPlace}`).join('\n')}\n\n**Gotra:** Kashyap | **Origin:** Lucknow/Kanpur → Pune`,
  ...
  if (q.includes('paternal side') || q.includes('dada') || q.includes('dadi') || q.includes('father side')) {
    ...
    return {
      content: `🌳 **Paternal Side of Your Family (${paternalMembers.length} members):**\n\n${paternalMembers.map(m => `• **${m.name}** (${m.relationship}) — ${m.currentPlace || m.birthPlace}`).join('\n')}\n\n**Gotra:** Bharadwaj | **Origin:** Varanasi → Nagpur → Pune`,
  ...
  if (q.includes('family history') || q.includes('tell me about') || q.includes('our story') || q.includes('history')) {
    ...
    return {
      content: `📖 **The Sharma-Mishra Family Story**\n\nYour family's recorded history spans **${2024 - (oldest.birthYear || 2000)} years**, from ${oldest.birthYear} to today.\n\n**The Beginning:** ${oldest.name} was born in ${oldest.birthPlace} in ${oldest.birthYear}. ${oldest.bio?.split('.')[0]}.\n\n**The Great Migration:** The Sharma family moved from Varanasi to Nagpur in 1970, when Suresh Kumar Sharma pursued education. The Mishras moved from Lucknow to Pune around the same era.\n\n**The Union:** In 1998, Vikram Sharma and Priya Mishra's wedding merged two great families — the Bharadwaj Sharmas of Varanasi and the Kashyap Mishras of Lucknow.\n\n**Today:** Your family spans **4 generations**, **${members.length} members**, across **6 cities** from Varanasi to Bengaluru. 🇮🇳\n\n*Generated by AI Family Copilot*`,
  ...
  if (q.includes('oldest') || q.includes('eldest') || q.includes('senior')) {
    ...
    return {
      content: `👴 **Oldest Family Member:**\n\n**${oldest.name}** — Born in ${oldest.birthYear} in ${oldest.birthPlace}\n\n${oldest.bio}\n\n*Gotra: ${oldest.gotra} | Status: ${oldest.isAlive ? '🟢 Alive' : '⚪ Deceased'}*`,
  ...
  if (q.includes('how many') || q.includes('total') || q.includes('count') || q.includes('size')) {
    ...
    return {
      content: `📊 **Family Statistics:**\n\n• **Total Members:** ${members.length}\n• **Living Members:** ${alive}\n• **Generations:** ${gen}\n• **Cities:** ${cities}\n• **Gotra (Paternal):** Bharadwaj\n• **Gotra (Maternal):** Kashyap\n\n**Members by Generation:**\n${[0, 1, 2, 3].map(g => `  Gen ${g}: ${members.filter(m => m.generation === g).length} members`).join('\n')}`,
      relatedIds: [],
    }
  }
...
const SUGGESTIONS = [
  { icon: GitBranch, label: "How am I related to Arjun?", color: "text-violet-400" },
  { icon: Users, label: "Show me maternal side", color: "text-green-400" },
  { icon: Search, label: "Who lives in Mumbai?", color: "text-blue-400" },
...
export default function AICopilotPage() {
  ...
  const sendMessage = useCallback(async (query: string) => {
    ...
    const { content, relatedIds } = generateAIResponse(query, members)
  ...
  const clearChat = () => {
    setMessages([{
      id: 'welcome-new',
      role: 'ai',
      content: `Chat cleared! Ask me anything about the Sharma-Mishra family. 🌳`,
      timestamp: new Date().toISOString(),
    }])
  }
